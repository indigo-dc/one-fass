...
void Configurator::print_loaded_options() {
        ...
        for (po::variables_map::iterator it = vm.begin();
             it != vm.end(); it++) {
                ...
                switch (tp) {
                        case is_string: os
                                        << boost::any_cast<string>(vm[opt_name].value());
                        break;
                        case is_int: os
	                                << boost::any_cast<int>(vm[opt_name].value());
                        break;
                        case is_double: os
                                        << boost::any_cast<double>(vm[opt_name].value());
                        break;
                        case is_bool: os
                                        << boost::any_cast<bool>(vm[opt_name].value());
                        break;
