...
main(int argc, char **) {
    ...
    try {
        ...
        string const methodName("piripicchiolo");

        xmlrpc_c::paramList sampleAddParms;
        //sampleAddParms.add(xmlrpc_c::value_int(5));
        //sampleAddParms.add(xmlrpc_c::value_int(7));
        sampleAddParms.add(xmlrpc_c::value_string("pippo"));
        sampleAddParms.add(xmlrpc_c::value_string("pluto"));
        ...
        string const value(xmlrpc_c::value_string(myRpcP->getResult()));
            // Assume the method returned an integer; throws error if not

        cout << "Result of RPC: " << value << endl;
